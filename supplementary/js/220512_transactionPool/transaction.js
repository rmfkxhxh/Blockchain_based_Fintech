...
import { getLatestBlock } from "./block.js";
...
const createTransactionFromUser = (amount, fromAddress, toAddress) => {
    // 1. 아직 처리되지 않았지만 트랜잭션 풀에 올라가 있는 내용을 확인
    ...
    const tx = new Transaction();
    tx.txIns = unsignedTxIns;
    tx.txOuts = createTxOuts(toAddress, amount, leftoverAmount); // 받는 사람 주소
    tx.id = getTransactionId(tx);
...
const isInTx = (txIn) => {
    ...
    return findTxOut !== undefined; //같은게 있다면 tr
}


const processTransaction = (transactions, unspentTxOuts, blockIndex) => {
    // 1. 예외처리 (트랜잭션 구조를 검증하는 과정) 
    // TODO: isValidBlockTransaction함수 만들기
    // if (!isValidBlockTransaction(transactions, unspentTxOuts, blockIndex)) {
    ...
    const newUnspentTxOuts = transactions.map((tx) => {
        // console.log(tx)
        return tx.txOuts.map((txOut) => { return new UnspentTxOut(tx.id, blockIndex, txOut.address, txOut.amount) })
    })
        .reduce((a, b) => a.concat(b), [])
    ...
    // console.log('un ', unspentTxOuts)
    // console.log('consum ', consumedTxOuts)
    // console.log('newunspent ', newUnspentTxOuts)
    ...
    // console.log(resultUnSpentTxOuts)
...
// unspentTxOuts.push(processTransaction(getLatestBlock().data, [], 0)); 
//  processTransaction() return하는 데이터는 UnspentTxOut[]
...
export {
    ...
    getTransactionId, getTxInAmount
}